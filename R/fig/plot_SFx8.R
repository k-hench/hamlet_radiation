...
data2 <- data %>% 
  ...
  rename(`Number~of~outlier~windows` = 'n',
         `Average~window~length~(kb)` = 'avg_length',
         `Cummulative~window~length~(Mb)` = 'overal_length') %>%
...
p <- ggplot(data2, aes(x = weighted, y = Value))+
  geom_hline(data = tibble(variable = c('Cummulative~window~length~(Mb)',
                                        'Average~window~length~(kb)',
  ...
  scale_x_continuous(expression(Global~weighted~italic(F[st])))+
  theme_minimal()+
