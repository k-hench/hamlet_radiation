...
# '2_analysis/summaries/fst_globals.txt')
# ----------------------------------------
# devtools::install_local('~/Desktop/GenomicOriginsScripts')
...
p_msmc <- msmc_data %>%
  ...
  labs(x="Generations Before Present", y = expression(Effective~Population~Size~(italic(N[e])))) +
...
p_cc <- cc_data %>%
  ...
  scale_x_log10(expand = c(0,0),
                labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  labs(x = "Generations Before Present", y = 'Cross-coalescence Rate') +
  coord_cartesian(xlim = c(250, 5*10^5)) +
