...
plot_network <- function(n_zeros, filtmode = "direct", filt = 0){
  ...
  data %>% 
    ...
    geom_node_point(aes(fill = spec,
                        shape = loc, color = after_scale(clr_darken(fill,.3))), size = 1) +
    ...
    scale_edge_alpha_continuous(limits = c(0,.1), guide = "none" ) +
...
plts <- c(c(5,8,6) %>% map(plot_network),
          c(5,8,6) %>% map(plot_network, filtmode = "bed95"),
          c(5,8,6) %>% map2(.y = list(c(0,4),
                                      c(0,4),
                                      c(0,4)),
                            plot_ibd_gw,  filtmode = "bed95"))
...
ggsave(plot = p_done,
       filename = "figures/SFx3_bed_filterd.png",
       width = f_width,
       height = .7 * f_width,
       type = "cairo")
