#!/usr/bin/env Rscript
# run from terminal:
# Rscript --vanilla R/fig/plot_SFY2.R \
...
p1 <- cowplot::plot_grid(
  grid::textGrob('Belize', gp = gpar(fontsize = plot_text_size * scl_title, col = clr_loc['bel'])),
  grid::textGrob('Honduras', gp = gpar(fontsize = plot_text_size * scl_title, col = clr_loc['hon'])),
  grid::textGrob('Panama', gp = gpar(fontsize = plot_text_size * scl_title, col = clr_loc['pan'])),
  ...
  get_legend(plot_list[[1]] + 
               guides(size = guide_legend(title = expression(Median~italic(F[ST])),
                                          title.position = "top",
                                          override.aes = list(color = "#9D9D9D"))) + 
               theme(legend.position = "bottom")),
  NULL,
  ncol = 3, rel_heights = c(.1,1,.2),
...
#                              rel_heights = c(1.6,1))
# export figure 1
hypo_save(p1, filename = 'figures/SFY2.pdf',
          width = f_width, 
          height =  f_width * .35,
          device = cairo_pdf,
