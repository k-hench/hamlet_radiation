...
# Analysis VI (Demographic History)

## Summary

The demographic hisotry rate is inferred within the [**nextflow**](https://www.nextflow.io/) script `analysis_msmc.nf` (located under `$BASE_DIR/nf/analysis_msmc/`), which runs on the _SNPs only_ data set.
...
The first part of the scripts inclues a large block of preparation work.
In this initial block, the data masks are being generated based in the samples coverages statistics cobined with the locations of idels and the refence genomes mappability. 

The whole script is opened by a creating a channel for the linkage groups since the coverage statisitcs are being created on a linkage group basis. 
...
To extract the seqencing depth for each individual, the unphased genotypes are opened as well (as this information is lost during pahsing).
...
The depth inforamtion is fed into a channel so that the information is accessible for `nextflow`.
...
Next, a channel is created from all the original `.bam` files from the mapped sequences ([git 1.6](genotyping-i-snps-only.html)).
...
The individual coverage statistics are then being querried using the individuals average depth to create the coverage mask for each individual.
...
To set up the `msmc2` runs, the sample grouping is wainig for the dummy process to finish.
...
Then, the specific `msmc2` input files are compiled from the combined masks of the involved samples (for each linkge group individually).
...
And finally `msmc2` is executed to inferr the demographic history of the involved samples.
...
The groups wait for the data preparation to fininsh.
...
And  `msmc2` is executed to inferr the cross-coalescence rate of the involved samples.
