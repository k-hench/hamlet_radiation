...
The second step of `FastEPRR` is parallelized over an arbitrary number of subprocesses.
Here, we initialize 250 parallel processes and combine the paralleliyation index with the results from step 1.
...
The file management of `nextflow` can be a bit complicated at times, so here duplicate the ouput of step 2 to later easily acess the paralleliyation indicees and the actual output.
